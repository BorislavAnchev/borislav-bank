...
const NewAccountPage = ({history}) => {
  ...
  return (
    <div data-testid='Create Account Component'>
      ...
      <Button onClick={() => handleAddAccount(iban, currency)} data-testid='Create Page Add Account Button'>Add</Button>
      <Link className="back" to="/"><Button data-testid='Back Button'>Back</Button></Link>
      <p className='validation-warning' data-testid='validation-warning'>{addAccountWarning}</p>
    </div>
  )
}

export default withRouter(NewAccountPage);